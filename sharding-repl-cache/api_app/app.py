...
async def startup():
    if REDIS_URL:
        # Используем RedisCluster вместо aioredis.from_url для поддержки кластера
        redis = RedisCluster.from_url(REDIS_URL, encoding="utf8")
...
async def root():
    ...
    for collection_name in collection_names:
        ...
        # Получаем общую статистику по коллекции, включая распределение по шардам
        stats = await db.command("collStats", collection_name)
        shard_counts = {}
        if "shards" in stats:
            for shard_id, shard_info in stats["shards"].items():
                shard_counts[shard_id] = shard_info.get("count")
    ...
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", {}):
            ...
            # Подсчитываем количество реплик в каждом шарде
            # Формат host: "replica_set_name/host1:port1,host2:port2,host3:port3"
            if "/" in shard_host:
                replica_set_hosts = shard_host.split("/")[1]
                replica_count = len(replica_set_hosts.split(","))
                replica_counts[shard_id] = replica_count
            else:
                replica_counts[shard_id] = 1

        # Получаем информацию о config server
        try:
            config_db = client.get_database("config")
            config_shards = await config_db.shards.find_one()
            if config_shards:
                # Получаем информацию о config server replica set
                config_servers_info = await client.admin.command("getCmdLineOpts")
                if "parsed" in config_servers_info and "sharding" in config_servers_info["parsed"]:
                    configdb = config_servers_info["parsed"]["sharding"].get("configDB", "")
                    if "/" in configdb:
                        config_hosts = configdb.split("/")[1]
                        replica_counts["config_server"] = len(config_hosts.split(","))
        except Exception as e:
            logger.warning(f"Could not get config server info: {e}")
    ...
    return {
        ...
        "shards": shards,
        "replica_counts": replica_counts if replica_counts else None,
